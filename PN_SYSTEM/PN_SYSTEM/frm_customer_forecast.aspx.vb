...
Public Class frm_customer_forecast
    ...
    Protected Sub gv_detail_RowCommand(sender As Object, e As GridViewCommandEventArgs) Handles gv_detail.RowCommand
        If e.CommandName = "Select" Then
            ...
            'Fetch value of Name.
            Dim name As String = TryCast(row.FindControl("lblmonth"), Label).Text
    ...
    Protected Sub gv_detail_Load(sender As Object, e As EventArgs) Handles gv_detail.Load
        itemtable = mdSQLCommand.load_cusforecast("", 0, 10)
        gv_detail.DataSource = itemtable
        gv_detail.DataBind()
    End Sub
    Sub show_data()
    ...
    Sub Export_Forecast_All()
        Try
            Dim sql As String
            Dim gridTemp As New GridView
            Dim DTExport As New DataTable

            Dim file_export_name As String = "Export_Forecast" & DateTime.Now.ToString("yyyyMMddHHmmss") & ".xls"
            sql = "SELECT  month,plant,customer_no as 'customer no',customer_name as 'customer name',customer_group as 'car maker',part_no as 'part no',part_no_sub as 'sub part no',mfg_no as 'mfg',cust_part_no as 'customer part',standard_pack_code as 'standard pack code',standard_pack_qty as 'standart pack qty',qty_forecast as 'qty',file_import as 'file_import',record_date as 'import date',update_by as 'update by'"
            sql &= " FROM  tblAVCustomersForecast"
            sql &= " WHERE flag = 1"
            ...
            End If
            sql &= " AND import_type='MULTI'"
            DTExport = TokaiwsSQL.SelectSQL(sql).Tables(0)
            If DTExport.Rows.Count <> 0 Then
                gridTemp.DataSource = DTExport
                gridTemp.DataBind()
            Else
            ...
            Response.AddHeader("content-disposition", "attachment; filename=" & file_export_name)
